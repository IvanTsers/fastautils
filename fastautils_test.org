...
#+begin_src latex
  ...
  To test \texttt{FindByHeader}, we create a slice of two short
  sequences with set names. Then we call the function with the pattern
  "target". We expect to get \texttt{seq2} as it contains the word
