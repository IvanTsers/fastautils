...
We check if the byte is a non-ambiguous nucleotide using an array that
we will initialize shortly.
#+end_export
...
#+end_src
#+begin_export latex
The array \ty{isACGT} stores \texttt{true} for bytes A, C, G, T, a, c,
g, t. The map is filled with values in the \texttt{init} function.
#+end_export
#+begin_src go <<Functions>>=
  var isACGT [256]bool
